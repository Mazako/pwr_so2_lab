...
#
start="Lp.;Nr albumu;Nazwisko;Imiona;Rok;Semestr;Przedmiot kształcenia;Ocena (np. 3.0);Data (RRRR-MM-DD);Komentarz"

cat dodatkowe/listaSluchaczy_E07-19x.csv  |\
grep -n "$start" |\
cut -d ":" -f 1  |\
xargs -I {} tail dodatkowe/listaSluchaczy_E07-19x.csv -n +{} |\
tail -n +2 |\
cut -d ";" -f 2 |\
rev | cut -c -6 | rev |\
xargs -I {} echo {}@student.pwr.edu.pl